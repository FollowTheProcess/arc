package syntax_test

import (
	"os"
	"path/filepath"
	"strconv"
	"strings"
	"testing"

	"go.followtheprocess.codes/arc/internal/syntax"
	"go.followtheprocess.codes/test"
)

// TestSpan tests the Span position calc logic by inserting start ('[')
// and end (']') markers in src and asserting the positions are correct.
//
// '[]' represents a point span (zero width) at the position of '['.
func TestSpan(t *testing.T) {
	tests := []struct {
		name  string          // Name of the test case
		src   string          // Source text under test, with the span bracketed by [ ]
		start syntax.Position // Expected position of the span start
		end   syntax.Position // Expected position of the span end
	}{
		{
			name:  "empty",
			src:   "[]",
			start: syntax.Position{Line: 1, Col: 1},
			end:   syntax.Position{Line: 1, Col: 1},
		},
		{
			name:  "point mid line 1",
			src:   "ab[]c\ndef\nghi",
			start: syntax.Position{Line: 1, Col: 3},
			end:   syntax.Position{Line: 1, Col: 3},
		},
		{
			name:  "point start of line 2",
			src:   "abc\n[]def\nghi",
			start: syntax.Position{Line: 2, Col: 1},
			end:   syntax.Position{Line: 2, Col: 1},
		},
		{
			name:  "point mid line 2",
			src:   "abc\nd[]ef\nghi",
			start: syntax.Position{Line: 2, Col: 2},
			end:   syntax.Position{Line: 2, Col: 2},
		},
		{
			name:  "point end of line 2",
			src:   "abc\ndef[]\nghi",
			start: syntax.Position{Line: 2, Col: 4},
			end:   syntax.Position{Line: 2, Col: 4},
		},
		{
			name:  "point start of last line",
			src:   "abc\ndef\n[]ghi",
			start: syntax.Position{Line: 3, Col: 1},
			end:   syntax.Position{Line: 3, Col: 1},
		},
		{
			name:  "point end of last line",
			src:   "abc\ndef\nghi[]",
			start: syntax.Position{Line: 3, Col: 4},
			end:   syntax.Position{Line: 3, Col: 4},
		},
		{
			name:  "point single line",
			src:   "ab[]c",
			start: syntax.Position{Line: 1, Col: 3},
			end:   syntax.Position{Line: 1, Col: 3},
		},
		{
			name:  "single line span",
			src:   "abc\nd[ef]\nghi",
			start: syntax.Position{Line: 2, Col: 2},
			end:   syntax.Position{Line: 2, Col: 4},
		},
		{
			name:  "two line span",
			src:   "ab[c\nde]f",
			start: syntax.Position{Line: 1, Col: 3},
			end:   syntax.Position{Line: 2, Col: 3},
		},
		{
			name:  "three line span",
			src:   "[abc\ndef\nghi]",
			start: syntax.Position{Line: 1, Col: 1},
			end:   syntax.Position{Line: 3, Col: 4},
		},
		{
			name:  "span ending at EOF",
			src:   "abc\nde[f]",
			start: syntax.Position{Line: 2, Col: 3},
			end:   syntax.Position{Line: 2, Col: 4},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, span := srcSpan(t, tt.src)

			test.Equal(t, span.Start(), tt.start, test.Context("wrong start of span"))
			test.Equal(t, span.End(), tt.end, test.Context("wrong end of span"))
		})
	}
}

func TestSnippet(t *testing.T) {
	tests := []struct {
		name         string // Name of the test case
		src          string // Source text under test, with the span bracketed
		want         string // Expected snippet
		contextLines int    // Number of context lines to include either side
	}{
		{
			name:         "empty",
			src:          "[]",
			contextLines: 1,
			want:         "",
		},
		{
			name:         "single line span no context",
			src:          "abc\nd[ef]\nghi",
			contextLines: 0,
			want:         "def\n",
		},
		{
			name:         "single line span with context",
			src:          "abc\nd[ef]\nghi",
			contextLines: 1,
			want:         "abc\ndef\nghi",
		},
		{
			name:         "multi line span no context",
			src:          "x\n[abc\ndef]\ny",
			contextLines: 0,
			want:         "abc\ndef\n",
		},
		{
			name:         "multi line span with context",
			src:          "x\n[abc\ndef]\ny",
			contextLines: 1,
			want:         "x\nabc\ndef\ny",
		},
		{
			name:         "context truncated at start of file",
			src:          "[abc]\ndef",
			contextLines: 5,
			want:         "abc\ndef",
		},
		{
			name:         "context truncated at end of file",
			src:          "abc\n[def]",
			contextLines: 5,
			want:         "abc\ndef",
		},
		{
			name:         "target last line no trailing newline",
			src:          "abc\nd[ef]",
			contextLines: 0,
			want:         "def",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, span := srcSpan(t, tt.src)

			got := span.Snippet(tt.contextLines)
			test.Diff(t, string(got), tt.want)
		})
	}
}

func TestPositionString(t *testing.T) {
	tests := []struct {
		want      string
		line, col int
	}{
		{line: 0, col: 0, want: "0:0"},
		{line: 1, col: 1, want: "1:1"},
		{line: 12, col: 37, want: "12:37"},
		{line: 99999, col: 99999, want: "99999:99999"},
	}

	for i, tt := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			pos := syntax.Position{Line: tt.line, Col: tt.col}
			got := pos.String()

			test.Equal(t, got, tt.want)
		})
	}
}

func TestSpanString(t *testing.T) {
	tests := []struct {
		name string
		want string
		span syntax.Span
	}{
		{
			name: "nil file",
			span: syntax.Span{},
			want: "",
		},
		{
			name: "start of file",
			span: syntax.Span{
				File:        syntax.NewSourceFile("test.http", []byte("hello\nthere")),
				StartOffset: 0,
				EndOffset:   0,
			},
			want: "test.http:1:1",
		},
		{
			name: "start of line 2",
			span: syntax.Span{
				File:        syntax.NewSourceFile("test.http", []byte("hello\nthere")),
				StartOffset: 6, // 't' in "there"
				EndOffset:   6,
			},
			want: "test.http:2:1",
		},
		{
			name: "mid line 2",
			span: syntax.Span{
				File:        syntax.NewSourceFile("test.http", []byte("hello\nthere")),
				StartOffset: 8,
				EndOffset:   8, // Doesn't matter, String() returns start
			},
			want: "test.http:2:3",
		},
		{
			name: "EOF position",
			span: syntax.Span{
				File:        syntax.NewSourceFile("test.http", []byte("hello")),
				StartOffset: 5, // one past 'o'
				EndOffset:   5,
			},
			want: "test.http:1:6",
		},
		{
			name: "multi-line span renders start only",
			span: syntax.Span{
				File:        syntax.NewSourceFile("test.http", []byte("hello\nthere\nworld")),
				StartOffset: 1,  // 'e' in "hello"
				EndOffset:   14, // 'r' in "world"
			},
			want: "test.http:1:2",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := tt.span.String()
			test.Equal(t, got, tt.want)
		})
	}
}

func TestDiagnosticString(t *testing.T) {
	tests := []struct {
		name string            // Name of the test case
		want string            // Expected result
		diag syntax.Diagnostic // Diagnostic under test
	}{
		{
			name: "empty",
			diag: syntax.Diagnostic{},
			want: "",
		},
		{
			name: "severity only",
			diag: syntax.Diagnostic{Severity: syntax.SeverityWarning},
			want: "",
		},
		{
			name: "valid but invalid severity",
			diag: syntax.Diagnostic{
				Severity: syntax.SeverityInvalid,
				Message:  "Uh oh!",
				Span: syntax.Span{
					File:        syntax.NewSourceFile("test.http", []byte("hello\nthere\nworld")),
					StartOffset: 1,  // 'e' in "hello"
					EndOffset:   14, // 'r' in "world"
				},
			},
			want: "",
		},
		{
			name: "valid warning",
			diag: syntax.Diagnostic{
				Severity: syntax.SeverityWarning,
				Message:  "Uh oh!",
				Span: syntax.Span{
					File:        syntax.NewSourceFile("test.http", []byte("hello\nthere\nworld")),
					StartOffset: 1,  // 'e' in "hello"
					EndOffset:   14, // 'r' in "world"
				},
			},
			want: "[warning] test.http:1:2: Uh oh!",
		},
		{
			name: "valid error",
			diag: syntax.Diagnostic{
				Severity: syntax.SeverityError,
				Message:  "This is broken",
				Span: syntax.Span{
					File:        syntax.NewSourceFile("test.http", []byte("hello\nthere")),
					StartOffset: 6, // 't' in "there"
					EndOffset:   6,
				},
			},
			want: "[error] test.http:2:1: This is broken",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := tt.diag.String()
			test.Equal(t, got, tt.want)
		})
	}
}

func BenchmarkNewSourceFile(b *testing.B) {
	file := filepath.Join("testdata", "bench", "lines.txt")
	data, err := os.ReadFile(file)
	test.Ok(b, err)

	for b.Loop() {
		syntax.NewSourceFile("bench", data)
	}
}

// srcSpan parses a source string in which the desired span is bracketed by
// '[' and ']'. The brackets are stripped, the line offset table is built, and
// a Span pointing at the bytes that were between the brackets is returned.
//
// '[]' means a zero-width point span.
func srcSpan(t *testing.T, src string) (*syntax.SourceFile, syntax.Span) {
	t.Helper()

	start := strings.IndexByte(src, '[')
	test.True(t, start >= 0, test.Context("no '[' in src"))

	rel := strings.IndexByte(src[start+1:], ']')
	test.True(t, rel >= 0, test.Context("no ']' after '[' in src"))
	end := start + 1 + rel

	content := src[:start] + src[start+1:end] + src[end+1:]
	file := syntax.NewSourceFile("test.http", []byte(content))

	return file, syntax.Span{
		File:        file,
		StartOffset: start,
		EndOffset:   end - 1, // -1: '[' has been stripped, shifting ']' left by one
	}
}
